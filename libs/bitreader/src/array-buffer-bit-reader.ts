export class ArrayBufferBitReader {
  ...
  align(): ArrayBufferBitReader {
    this._trash(this.bitsRemaining & 0b111);
    return this;
  }

  seek(offset: number): ArrayBufferBitReader {
    this.idx = offset;
    this.bitBuffer = 0;
    this.bitsRemaining = 0;
    return this;
  ...
  skip(n: number): ArrayBufferBitReader {
  ...
  private _trash(n: number): ArrayBufferBitReader {
    this.bitBuffer <<= n;
    this.bitsRemaining -= n;
    return this;
  }

  private _fill(n: number): ArrayBufferBitReader {
